// Format reference: GARbro ArcFormats/Basil/ArcMIF.cs
...
const SIGNATURE = Buffer.from("MIF\0", "binary");
const HEADER_SIZE = 8;
...
export const mifDescriptor: FormatDescriptor = {
	id: "basil-mif",
	name: "BasiL resource archive",
	extensions: [],
	capabilities: {
		detect: true,
		list: true,
	...
	attribution: [
		{
			project: "GARbro",
			source: "ArcFormats/Basil/ArcMIF.cs",
			license: "MIT",
...
async function readMif(source: ByteSource): Promise<{
	entries: FixedEntry[];
	metadata: Record<string, unknown>;
}> {
	const count = await parseHeader(source);
	if (count === undefined) {
		throw new GarbroError("INVALID_ARCHIVE", "Invalid BasiL MIF signature");
	}
	const indexSize = count * RECORD_SIZE;
	if (BigInt(HEADER_SIZE + indexSize) > source.size) {
		throw new GarbroError("INVALID_ARCHIVE", "BasiL MIF index is truncated");
	}
	...
	for (let id = 0; id < count; id += 1) {
		...
		if (name.length === 0) {
			throw new GarbroError(
				"INVALID_ARCHIVE",
				"BasiL MIF entry has an empty name",
			);
		}
		...
		if (!checkPlacement(offset, size, source.size)) {
			throw new GarbroError(
				"INVALID_ARCHIVE",
				`BasiL MIF entry points outside the archive: ${name}`,
			);
		}
		entries.push(
			createFixedEntry({
...
export const mifFormat: ArchiveFormat = defineFixedArchive({
	descriptor: mifDescriptor,
