// The tables of the cipher of the GPlay engine, of "Legacy/GPlay/ArcYSK.cs".
// GARbro commit b09ee4570ccb1daf6ac56710ee8934dc0b8baeb0, MIT License.

/** `DesTransform.pShift0`. */
export const PSHIFT0: readonly number[] = [
	56, 48, 40, 32, 24, 16, 8, 0, 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34,
	26, 18, 10, 2, 59, 51, 43, 35,
];

/** `DesTransform.pShift1`. */
export const PSHIFT1: readonly number[] = [
	62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 60, 52, 44, 36,
	28, 20, 12, 4, 27, 19, 11, 3,
];

/** `DesTransform.pShift2`. */
export const PSHIFT2: readonly number[] = [
	1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1,
];

/** `DesTransform.pShift3`. */
export const PSHIFT3: readonly number[] = [
	13, 16, 10, 23, 0, 4, 2, 27, 14, 5, 20, 9, 22, 18, 11, 3, 25, 7, 15, 6, 26,
	19, 12, 1, 40, 51, 30, 36, 46, 54, 29, 39, 50, 44, 32, 47, 43, 48, 38, 55, 33,
	52, 45, 41, 49, 35, 28, 31,
];

/** `DesTransform.pState`. */
export const PSTATE: readonly number[] = [
	0, 8, 4, 12, 2, 10, 6, 14, 1, 9, 5, 13, 3, 11, 7, 15,
];

/** `DesTransform.st0`. */
export const ST0: readonly number[] = [
	10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8, 13, 7, 0, 9, 3, 4, 6,
	10, 2, 8, 5, 14, 12, 11, 15, 1, 13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10,
	14, 7, 1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12,
];

/** `DesTransform.st1`. */
export const ST1: readonly number[] = [
	15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10, 3, 13, 4, 7, 15, 2, 8,
	14, 12, 0, 1, 10, 6, 9, 11, 5, 0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3,
	2, 15, 13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9,
];

/** `DesTransform.st2`. */
export const ST2: readonly number[] = [
	2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9, 14, 11, 2, 12, 4, 7, 13,
	1, 5, 0, 15, 10, 3, 9, 8, 6, 4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0,
	14, 11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3,
];

/** `DesTransform.st3`. */
export const ST3: readonly number[] = [
	12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11, 10, 15, 4, 2, 7, 12, 9,
	5, 6, 1, 13, 14, 0, 11, 3, 8, 9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13,
	11, 6, 4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13,
];

/** `DesTransform.st4`. */
export const ST4: readonly number[] = [
	7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15, 13, 8, 11, 5, 6, 15, 0,
	3, 4, 7, 2, 12, 1, 10, 14, 9, 10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2,
	8, 4, 3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14,
];

/** `DesTransform.st5`. */
export const ST5: readonly number[] = [
	13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7, 1, 15, 13, 8, 10, 3, 7,
	4, 12, 5, 6, 11, 0, 14, 9, 2, 7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3,
	5, 8, 2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11,
];

/** `DesTransform.st6`. */
export const ST6: readonly number[] = [
	14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7, 0, 15, 7, 4, 14, 2, 13,
	1, 10, 6, 12, 11, 9, 5, 3, 8, 4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10,
	5, 0, 15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13,
];

/** `DesTransform.st7`. */
export const ST7: readonly number[] = [
	4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1, 13, 0, 11, 7, 4, 9, 1,
	10, 14, 3, 5, 12, 2, 15, 8, 6, 1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5,
	9, 2, 6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12,
];

/** `DesTransform.tShift0`. */
export const TSHIFT0: readonly number[] = [
	57, 49, 41, 33, 25, 17, 9, 1, 59, 51, 43, 35, 27, 19, 11, 3, 61, 53, 45, 37,
	29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7,
];

/** `DesTransform.tShift1`. */
export const TSHIFT1: readonly number[] = [
	56, 48, 40, 32, 24, 16, 8, 0, 58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36,
	28, 20, 12, 4, 62, 54, 46, 38, 30, 22, 14, 6,
];

/** `DesTransform.tMask0`. */
export const TMASK0: readonly number[] = [
	2147483648, 1, 2, 4, 8, 16, 8, 16, 32, 64, 128, 256, 128, 256, 512, 1024,
	2048, 4096, 2048, 4096, 8192, 16384, 32768, 65536, 32768, 65536, 131072,
	262144, 524288, 1048576, 524288, 1048576, 2097152, 4194304, 8388608, 16777216,
	8388608, 16777216, 33554432, 67108864, 134217728, 268435456, 134217728,
	268435456, 536870912, 1073741824, 2147483648, 1,
];

/** `DesTransform.tMask1`. */
export const TMASK1: readonly number[] = [
	32768, 64, 524288, 1048576, 268435456, 2048, 134217728, 65536, 1, 16384,
	4194304, 33554432, 16, 131072, 1073741824, 512, 2, 128, 8388608, 8192,
	2147483648, 67108864, 4, 256, 262144, 4096, 536870912, 32, 2097152, 1024, 8,
	16777216,
];

/** `DesTransform.tShift2`. */
export const TSHIFT2: readonly number[] = [
	39, 7, 47, 15, 55, 23, 63, 31, 38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13,
	53, 21, 61, 29, 36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27,
	34, 2, 42, 10, 50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25, 32, 0, 40, 8, 48,
	16, 56, 24,
];
