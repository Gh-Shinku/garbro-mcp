...
/**
 * Wraps top down eight bit pixels in a grey bitmap. Bitmap rows are aligned to four bytes, so every
 * row is padded with zeros to `width` rounded up to a multiple of four; that padding is the only
 * difference from the stored pixels.
 */
