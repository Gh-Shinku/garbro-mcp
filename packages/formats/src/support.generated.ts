...
export const formatSupportCatalog = {
	...
	implementations: [
		...
		},
		{
			...
			unsupported: [
				"image creation",
				"the four further packed walks, those of the ways 0x04, 0x05, 0x0b and 0x0d, which are refused by name: each builds the reference table of sixty four thousand entries first, and all of that code is decompiler output with nothing but the reference to check a transcription against",
			],
			remainingVerification: [
				"real-game GARbro differential output",
				"of the five ported ways, the later attempts' shapes of back reference and the retry between attempts, which no fixture here reaches: the ones the fixtures pin are the first attempt's byte form and a counted run",
				"the four further packed walks, which no fixture here reaches because this port refuses them",
