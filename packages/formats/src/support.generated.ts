...
export const formatSupportCatalog = {
	...
	implementations: [
		...
		},
		{
			...
			unsupported: [
				"image creation",
				"the six further packed walks (0x04 to 0x0f), which are refused by name: each builds the reference table of sixty four thousand entries first, and all of that code is decompiler output with nothing but the reference to check a transcription against",
			],
			remainingVerification: [
				"real-game GARbro differential output",
				"of the four ported walks, the later attempts' shapes of back reference and the retry between attempts, which no fixture here reaches: the ones the fixtures pin are the first attempt's byte form and a counted run",
				"the six further packed walks, which no fixture here reaches because this port refuses them",
