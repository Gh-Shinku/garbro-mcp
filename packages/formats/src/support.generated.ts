...
export const formatSupportCatalog = {
	...
	implementations: [
		...
		},
		{
			...
			unsupported: [
				"image creation",
				"the nine packed walks of the reference (UnpackV0 through UnpackVD for the section ways 0x00 to 0x0f), which are refused by name: they are decompiler output with nothing but the reference to check a transcription against",
			],
			remainingVerification: [
				"real-game GARbro differential output",
				"the nine packed walks, which no fixture here reaches because this port refuses them",
				"a real picture, which would show which of the nine ways the engine writes and whether it pads its rows as the head says",
			],
		},
	],
} as const;
