...
- `WBM` (`ArcFormats/WildBug/ImageWBM.cs`, 1165 lines) is the picture beside the **ported** `WWA` sound, and
  ...
  the reference also reads as it stands, and the **`0x00` packed walk** as well. What remains is the **eight
  further packed walks** of the `WbmReader`, `UnpackV1` through `UnpackVD`, for the section ways `0x01` to
  `0x0F`. Each builds a reference table of
