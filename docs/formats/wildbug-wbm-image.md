...
Which walk a section asks for is worked out the reference's own way, by asking for the bits of the way in
turn: `0x00` and `0x02` reach the `0x00` and `0x02` walks, `0x01` and `0x03` the `0x01` and `0x03` walks,
`0x04` and `0x06` the `0x04` walk, `0x05` and `0x07` the `0x05` walk, `0x08` and `0x09` the `0x09` walk,
`0x0A` and `0x0B` the `0x0B` walk, and `0x0C` upwards the `0x0D` walk. The walks that remain - `0x04`, `0x05`,
`0x0B` and `0x0D` - are refused with `UNSUPPORTED_FEATURE`, and the message names the walk the section's byte
asks for, so a caller learns which one is missing rather than being handed a wrong picture. Each of them
builds a further table of sixty four thousand entries (`BuildTable`) before walking the picture; all of that
code is decompiler output in the reference (`sub_40919C`, `sub_46C26C`), so a port has nothing to check its
own transcription against but the reference itself.

## Deviations from the reference

* The four further packed walks - those of the ways `0x04`, `0x05`, `0x0B` and `0x0D` - are refused rather
  than ported, as above.
...
Eighteen tests build files with a mirror writer. The first eight: a stored picture of twenty four bits whose padded rows come out
...
Ten further tests cover the packed walks, and the bit stream they build is the one the walks read: a reader
...
whose counted run is added to the run itself; and then two of the `0x08` and `0x09` way, one for each of its
two shapes of reference.

What stands on the reference alone: the four further packed walks, which this port refuses; of the five ways
