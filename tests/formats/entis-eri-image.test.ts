...
import { ErisaEncoder, addSymbolBits, bitsToBuffer } from "../helpers/erisa.js";
...
function losslessFrame(input: {
	...
	for (const block of input.blocks) {
		if (input.operationTree) {
			...
			addSymbolBits(bits, new ErisaHuffmanTree(), 0xc0);
		}
...
function frameSection(places: Buffer): Buffer {
	return section("ImageFrm", places);
}
